...
const VerifySearchHeader = ({ workerSearch, setWorkerSearch, isSearching, error, handleSearchSubmit, t }) => (
  ...
  >
    ...
    <h1 className="font-clash text-4xl md:text-5xl font-bold tracking-tighter mb-3 text-gradient">
    ...
    {/* Search Form */}
    <form onSubmit={handleSearchSubmit} className="max-w-2xl mx-auto glass-card p-5 rounded-xl" aria-label={t('verify.searchFormLabel', 'Search by wallet address')}>
      <div className="flex items-center border-b border-white/10 focus-within:border-green-400/50 transition-colors duration-300">
        <Search className="w-5 h-5 text-green-400/40 mr-3" aria-hidden="true" />
        <input
          type="text"
        ...
        <button type="submit" disabled={isSearching || !workerSearch}
          ...
          className="btn-glow px-6 py-2.5 rounded-lg font-bold text-[11px] uppercase tracking-wider transition-all disabled:opacity-30 disabled:shadow-none flex items-center gap-2 ml-3">
    ...
    <AnimatePresence>
      {error && (
        <motion.p initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
          role="alert"
          className="mt-4 text-red-400/80 text-xs flex items-center justify-center gap-2 border border-red-400/20 bg-red-400/[0.05] backdrop-blur-sm px-4 py-2.5 rounded-lg w-fit mx-auto font-inter">
