  ...
  return (
    <>
          ...
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
              ...
              <div className="lg:col-span-4 space-y-4">
                <div className="border border-white/[0.07] rounded-[2px] bg-white/[0.02] overflow-hidden" role="region" aria-label={t('verify.profileCard', 'Worker profile')}>
                  {isSearching ? (
                    ...
                    <div className="p-6">
                      <div className="text-center mb-5">
                        <div className="w-14 h-14 rounded-[2px] bg-white/5 border border-white/10 flex items-center justify-center mx-auto mb-3">
                          <User className="w-7 h-7 text-white/30" aria-hidden="true" />
                        </div>
              ...
              <div className="lg:col-span-8 space-y-4">
                {/* Reputation */}
                <div className="border border-white/[0.07] rounded-[2px] bg-white/[0.02]" role="region" aria-label={t('verify.reputationRegion', 'Reputation score and breakdown')}>
                  {isSearching ? (
                ...
                {/* Stats Row */}
                {!isSearching && profile && (
                  <div className="grid grid-cols-3 gap-px bg-white/5" role="region" aria-label={t('verify.statsRegion', 'Worker statistics')}>
                    {[
                ...
                <div role="region" aria-label={t('verify.endorsementHistory', 'Endorsement history')}>
                  <div className="flex items-center gap-2 mb-4">
                    <div className="w-1 h-5 bg-white/20 rounded-full" aria-hidden="true" />
                  ...
                  <div className="space-y-2" role="list" aria-label={t('verify.endorsementsList', 'List of endorsements')}>
                    {isSearching ? (
                          ...
                          <div className="flex items-start justify-between mb-2">
                            <div className="flex items-center gap-3">
                              <div className="w-7 h-7 rounded-[2px] bg-white/5 border border-white/10 flex items-center justify-center">
                                <ShieldCheck className="w-3.5 h-3.5 text-white/30" aria-hidden="true" />
                              </div>
                              <div>
                            ...
                            <div className="flex gap-0.5" role="img" aria-label={`${endorsement.rating} out of 5 stars`}>
                              {[1,2,3,4,5].map(s => (<Star key={s} className={`w-2.5 h-2.5 ${s <= endorsement.rating ? 'text-white fill-white' : 'text-white/10'}`} aria-hidden="true" />))}
                            </div>
                          </div>
                          <div className="mb-2">
                      ...
                      <div className="p-10 text-center border border-dashed border-white/5 rounded-[2px]" role="status">
      ...
      {!isSearching && !profile && !error && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="text-center max-w-sm mx-auto" role="status" aria-label={t('verify.emptyState', 'No search performed yet')}>
          <div className="w-14 h-14 rounded-[2px] bg-white/[0.03] border border-white/5 flex items-center justify-center mx-auto mb-4">
            <Sparkles className="w-6 h-6 text-white/10" aria-hidden="true" />
          </div>
      ...
      {/* Footer badges */}
      <div className="mt-10 flex items-center justify-center gap-5 text-white/10" role="contentinfo" aria-label={t('verify.footerBadges', 'Security features')}>
        {[
          ...
          <React.Fragment key={i}>
            {i > 0 && <div className="w-1 h-1 rounded-full bg-white/5" aria-hidden="true" />}
